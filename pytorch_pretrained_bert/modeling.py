...
class BertSumConcatTopHiddenEmbeddingsPooler(nn.Module):

    def __init__(self, num_top=1, divide_by_sqrt=False, use_cls=False):
        """
        :param num_top: Number of top encoding layers to concatenate.
        :param divide_by_sqrt: Whether to divide the sum of embeddings by sqrt of sequence length. If False will divide by
        sequence length (computes mean) if True will divide by sqrt of sequence length.
        :param use_cls: Whether to use cls as part of pooling.
        """
        super().__init__()
        self.num_top = num_top
        self.divide_by_sqrt = divide_by_sqrt
        self.use_cls = use_cls

    def forward(self, encoded_layers, attention_mask):
        hidden_states = torch.cat(encoded_layers[-self.num_top:], dim=2)

        start_index = 0 if self.use_cls else 1
        relevant_hidden_states = hidden_states[:, start_index:]
        attention_mask = attention_mask[:, start_index:].to(torch.float)
        sequence_lengths = attention_mask.sum(dim=1, keepdim=True)

        embeddings_sum = (relevant_hidden_states * attention_mask.unsqueeze(2)).sum(dim=1)
        if self.divide_by_sqrt:
            return embeddings_sum / torch.sqrt(sequence_lengths)
        return embeddings_sum / sequence_lengths


class BertMaxConcatTopHiddenEmbeddingsPooler(nn.Module):

    def __init__(self, num_top=1, use_cls=False):
        """
        :param num_top: Number of top encoding layers to concatenate.
        :param use_cls: Whether to use cls as part of pooling.
        """
    ...
    def forward(self, encoded_layers, attention_mask):
        ...
        start_index = 0 if self.use_cls else 1
        relevant_hidden_states = hidden_states[:, start_index:]
        attention_mask = attention_mask[:, start_index:].to(torch.float).unsqueeze(2)

        relevant_hidden_states[attention_mask.expand_as(relevant_hidden_states) == 0] = -float("inf")  # Before max pooling turn masked values to negative inf instead of 0
        max_pooled_embeddings = relevant_hidden_states .max(dim=1)[0]
        return max_pooled_embeddings


class BertSumMeanTopHiddenEmbeddingsPooler(nn.Module):
    ...
    def forward(self, encoded_layers, attention_mask):
        stacked_embedding_states = torch.stack(encoded_layers[-self.num_top:], dim=3)
        hidden_states = stacked_embedding_states.mean(dim=3)
...
class BertMaxMeanTopHiddenEmbeddingsPooler(nn.Module):
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, output_all_encoded_layers=True, head_mask=None):
        ...
        pooled_output = self.pooler(encoded_layers, attention_mask)
        if not output_all_encoded_layers:
...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, head_mask=None):
        ...
        if self.output_attentions:
            ...
            _, pooled_output = outputs
        pooled_output = self.dropout(pooled_output)  # todo maybe should remove this dropout for siamese???
