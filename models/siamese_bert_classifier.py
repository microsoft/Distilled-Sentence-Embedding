...
class SiameseBertClassifier(nn.Module):
    ...
    def __init__(self, bert_for_sequence_embedding, siamese_head):
        super().__init__()
        self.bert = bert_for_sequence_embedding
        self.siamese_head = siamese_head

    def forward(self, first_input_ids, first_input_mask, second_input_ids, second_input_mask):
        first_embedding = self.bert(first_input_ids, attention_mask=first_input_mask)
        second_embedding = self.bert(second_input_ids, attention_mask=second_input_mask)
